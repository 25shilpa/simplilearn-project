...
public class FileUtil {
	...
	public List<MyFile> getFiles() // Creating method "getFiles" with the type "List<MyFile>" 
	{
		...
		File f = new File("d://JavaFiles");  //storing file directory or path in f object
	...
	{
		File f = new File("d://JavaFiles//"+fileName+".txt"); // Creating object "f" to store path, name of file (input from user) & file extension(.txt)
	...
	public boolean deleteFile(String fileName) // Creating "deleFile" method (boolean type) which takes string input
	{
		...
		for(MyFile file:files) // using for each loop to go through "files"
		{
			...
			{
				File f = new File("d://JavaFiles//"+file.getFileName()); // Creating "f" object which stores return value from "getFileName" method
